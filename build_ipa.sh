...
# TESTFLIGHT_API_TOKEN='XXX'
# TESTFLIGHT_TEAM_TOKEN='XXX'
# TESTFLIGHT_DISTRIBUTION_LISTS='XXX'
...
#	ssh $SSH_ARGS $REMOTE_HOST "cd $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/$HG_HASH; tar -xf $PAYLOAD_FILENAME; rm $PAYLOAD_FILENAME; rm -f $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/latest; ln -s $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/$HG_HASH $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/latest"
	...
	curl http://testflightapp.com/api/builds.json -F "file=@$IPA_FILENAME" -F api_token="$TESTFLIGHT_API_TOKEN" -F team_token="$TESTFLIGHT_TEAM_TOKEN" -F notes="$BUILD_CHANGES" -F notify=True -F distribution_lists="$TESTFLIGHT_DISTRIBUTION_LISTS"
