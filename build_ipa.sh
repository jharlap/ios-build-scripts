...
for APP_FILENAME in *.app; do
	...
	python -c "$MANIFEST_SCRIPT" -f "$APP_FILENAME" -d "$ROOT_DEPLOYMENT_ADDRESS/$APP_NAME/$GIT_HASH/$MANIFEST_OUTPUT_MANIFEST_FILENAME" -a "$ARCHIVE_FILENAME" -c "$BUILD_CHANGES"
	...
	ssh $REMOTE_HOST "cd $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/$HG_HASH; tar -xf $PAYLOAD_FILENAME; rm $PAYLOAD_FILENAME; rm -f $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/latest; ln -s $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/$HG_HASH $REMOTE_PARENT_PATH/${QUOTE}$APP_NAME${QUOTE}/latest"
